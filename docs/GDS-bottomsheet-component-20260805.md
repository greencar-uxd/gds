# Bottom sheet 컴포넌트 실측 · 정본 대조 — 마케팅 배포물 검토에서 파생
...
**저장소 정본(`data/foundation-data.json` → `canon`)과의 대조 결과**를 기록합니다.

산출물 3개를 이 저장소에 넣습니다.

| 파일 | 내용 |
|---|---|
| `data/component-bottom-sheet.json` | Bottom sheet · coupon · Large 버튼 실측 스펙 + 정본 토큰 매핑 |
...
새 스펙의 모든 색·간격·타이포·반경이 정본 안에 있는지 기계 검사합니다.
...
실측치를 `canon`과 대조했습니다. **간격·반경·타이포·색 모두 정본 안에 있습니다.**
...
**Figma 로컬 Text Style(22개)과 정본(21토큰)은 다른 체계입니다.** `[팩트]`
로컬은 `noto_sans/{역할}/{weight}` 이고 이름에 사이즈가 없어 `noto_sans/title/medium`이 8개(24~10px) 중복합니다.
정본은 사이즈별로 이름이 갈리므로(`Title 1`~`Title 8`) **이 중복은 정본에서 이미 해소된 상태**입니다. `[해석]`

한 건 미확정 — **16/700 정본이 `Display 3`과 `Heading 3` 둘 다**라 CTA 라벨이 어느 쪽인지 정할 수 없습니다. `[미확인]`
...
`Elevation_1`~`6` 재넘버링된 정본 스케일과 `Bottom Sheet`의 관계는 문서에 없습니다. `[미확인]`
...
| # | 항목 | 상태 |
|---|---|---|
| 1 | `#D9DDDF`(System/Input Line) 정본 편입 또는 Gray 020 흡수 | 판단 필요 |
...
| 6 | line-height 정본 규칙 신설 (현재 21토큰 전부 Auto) | 결정 필요 |
