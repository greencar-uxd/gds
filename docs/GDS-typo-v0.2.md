...
> **원본 21단계가 유일한 기준입니다.** 레거시 텍스트 스타일은 원본으로 통폐합될 대상이라 이 문서의 판단 근거가 아닙니다.
> **근거.** `GDS(Greencar Design System).fig` (export 2026-07-23) 실측 + 원본 Typography system 페이지의 Usage 열.
> **생성.** `npm run audit`
...
1. **자간(letterSpacing)은 원본에 정의가 없습니다** (정의 0건). 용도로 구분이 되므로 당장 막히지는 않지만, 값 차원의 구분이 필요해지면 여기부터 정해야 합니다.
2. **원본 단계별 실사용 횟수를 세지 않았습니다** `[미확인]`.
3. `.fig` 스냅샷 2026-07-23 기준입니다.
4. 원본 페이지에는 원래 폰트·용도 지정이 없었고, 이번 결정으로 채웠습니다 — 원본 `.fig` 반영은 디자인팀 몫입니다.
