...
원본 `.fig` 자체의 기록 16건(스와치 캡션 HEX 오류 2건, `Success` 이름 중복, `#00000` 5자리 오타 3곳, 복붙 잔재 레이어명, `Info_Box` 스와치 값 미적용)은 `data/color-decisions.json` 의 `sourceDefects` 에 노드 ID 와 함께 기록만 해 뒀습니다. 토큰 값에는 영향이 없습니다.
