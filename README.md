...
중간 9건 — **`Bottom Sheet`/`bottom sheet` 대소문자 충돌(`GAP-31`)** · **✅ 페이지 간격 주석 192건 중 21건이 스케일 밖(`GAP-30`)** · 같은 값 두 이름 6쌍 · ODA 색이 Badge/Brand 로 분리 · Icon 규칙이 두 곳으로 흩어짐 · ✅ 페이지끼리 AOS/Web 폭 불일치(360 vs 365) · ✅ 인데 미해결 메모가 남은 페이지 · Bottom sheet 정의가 Picker ✅ 안에 · **Title 8단계 중 2개가 라이브러리에 없음**
...
| `deprecated` | 2 | 원본이 `(X)` 로 표시한 것. 정본에서 제외 |
