...
## 원본이 무엇인가 (2026-08-06 확정)

> **원본 = Figma 파일 `Gx9UHfQdhSdHzr1j8Kp1Ab` «GDS (G car Design System)» 그 자체.**
> 회사 계정이 MCP 로 연동돼 있어 이 파일 하나를 직접 읽습니다. 내보내기·디코더·제한된 보기 권한을 거치지 않습니다.

`https://www.figma.com/design/Gx9UHfQdhSdHzr1j8Kp1Ab/GDS-G-car-Design-System-`

이 정의에서 따라오는 세 가지입니다.

- **다른 디자인 시스템은 보지 않습니다.** 레거시 라이브러리를 뒤져 이름이 비슷한 것을 가져오면 오염입니다 —
  실제로 `bottom sheet` 효과가 `작업 파일 공유` 라이브러리에서 섞여 들어와 토큰으로 나가고 있었습니다(GAP-31).
- **등록된 에셋·스타일 목록만 보지 않습니다. 페이지의 «본문»을 읽습니다.**
  `🚧` 는 «내용이 없다»는 뜻이 아닙니다 — Icon system 페이지가 🚧 인데도 이름 규칙·아이콘 184종·크기 단계·
  제작 가이드라인 `00_Size`~`08_Color` 를 전부 갖고 있었습니다. 여기서 **두 번** 틀렸습니다:
  ① 페이지를 안 열어서 3건, ② 열고도 노드 «이름»만 훑어서 2건 — 합쳐 **5건을 철회**했습니다(GAP-22).
- **모자란 곳은 순서대로 채웁니다** — ① 원본 다른 페이지 → ② 레거시 자료·G car 홈페이지 → ③ 레퍼런스 사이트(`data/reference-sites.json`).
  ②③ 에서 온 것은 «참고» 로 표시해 원본에서 온 것과 섞지 않습니다.

「2026-08-05 까지는 원본을 «GDS 라이브러리 + ✅ 페이지» 로 좁게 잡았습니다. 그 범위 안의 판정은 아래 그대로 유효하고,
바뀐 것은 **범위 밖도 원본이라는 것**입니다.」

원본 파일에는 라이브러리가 **6개** 물려 있습니다. GDS 라이브러리는 그중 하나뿐입니다.
...
**색 판정 기준은 색차(ΔE)가 아니라 라이브러리 소속입니다.** GDS 라이브러리에 없으면 원본이 아니고, 레거시의 값·이름은 판단 근거로 쓰지 않습니다.

이전에는 ✅ Color system 페이지의 **스와치 그림**에서 원본을 뽑았습니다. 그림에 그려지지 않은 7종이 빠져 있었습니다 — `Badge/ODA` · `Map Marker/Active`(흰색) · `Brand/ODA Line` · `System/Dim Layer` · `System/Info Box BG` · `System/Input Line` · `Map Marker/Shadow`. **원본은 53종이 아니라 60종입니다.**

레거시 649개 스타일 기반의 통폐합 판정·orphan 묶음·`CQ-4/5/6/11` 은 전제가 레거시였으므로 **전량 철회**했습니다.

원본 인벤토리는 `data/gds-library.json`, 조회 방법은 그 파일의 `method` 에 있습니다.

---

## 디자인 시스템으로서 모자란 곳

원본을 옮겨 적는 것이 아니라, **✅ 항목이 디자인 시스템으로서 모자란 곳을 찾아 메우는 것**이 이 저장소의 일입니다. `data/gds-gaps.json` 에 기록했고 `/gds/decisions` 에서 볼 수 있습니다.
...
| `GAP-9` | `(X)` 폐기 표시 스타일 2종을 원본에서 제외 |
| `GAP-11` | 반경 7단계 값 실측 |
| **`GAP-1`·`GAP-2`·`GAP-3`** | **텍스트 스타일 21종 정의** — `data/typography-library.json`. 라이브러리 5종은 스타일이 아니라 **그룹**이었고 이름에 단계 번호가 빠져 충돌한 것이었습니다 |
| **`GAP-15`** | **효과를 네 축으로 갈랐습니다** — `elevation` 6 · `component` 7 · `material` 1 · `deprecated` 2. 값이 측정된 3종을 `--gds-effect-*` 로 내보냅니다. 대소문자 충돌 1쌍은 값이 달라 합칠 수 없어 `GAP-31` 로 뗐습니다 |
...
| **`GAP-12`** | **3계층이 어디까지 서는지 확정** — 색 60+11 · 타이포 21+8 · 간격 14+Guidelines 10. **간격의 시맨틱 계층은 «없음»으로 확정**했고, 그게 우리 누락이 아니라 원본에 근거가 없어서라는 것을 `tools/spacing-census.js` 로 입증했습니다 (✅ 페이지 5곳 · 주석 192건) |
| **`GAP-27`** | **Rubik 은 원본에 없습니다** — `Typography system` ✅ 페이지 레이어 이름 3,492개 중 Rubik 0건. `TQ-6`(Noto Sans KR 단일) 유지, Picker 본문 쪽이 낡은 참조라 `SD-18` 로 기록 |

| **`GAP-34`** | **Text field 를 «원본 대기»로 비워 두고 있었습니다** — 🚧 페이지를 열어 보니 정의·유형 4가지·유형별 구조·스페이싱·변형 42종이 있었습니다. 페이지 자체의 결함 5건도 함께 기록 |
| **`GAP-35`** | **Radius 페이지에 KRDS 형태 가이드가 프레임째 붙어 있습니다** — 인용임을 밝히고 값은 편입하지 않았습니다. 원본이 적어 둔 «10 → 8» 미결 메모 5건도 그대로 옮김 |
| `GAP-10` | 그림자 체계 밖 효과 이름 정리 — 원본 Elevation 페이지도 «표준 / 별도 지정»으로 같은 구분을 쓰고 있음을 확인 |

### 남은 것 10건

**높음 2건**

- **`GAP-32` — 🚧 페이지를 «내용이 없는 페이지»로 세어 왔습니다.** 등록된 스타일·에셋 목록만 보고 페이지를 열지 않은 것이 원인입니다. Icon system · Text field · Elevation system 셋 다 내용이 있었습니다. 🚧 19쪽 중 **4쪽 확인**, 15쪽 남음.
- **`GAP-33` — Elevation 레벨 대응표가 없습니다.** 페이지는 Level 0~5(그림자 5단계), 라이브러리는 `Elevation_1~6`. 어느 스타일이 어느 레벨인지 원본이 적어 두지 않았고, 값도 어긋납니다(페이지 1겹 / 라이브러리 2겹). 이름의 숫자만 보고 짝지어 덮어쓰지 않습니다.

중간 4건 — 32px 아이콘 굵기가 원본 안에서 1.6/1.5 두 값(`GAP-36`) · ✅ 인데 미해결 메모가 남은 페이지(`GAP-25`) · Title 8단계 중 2개가 라이브러리에 없음(`GAP-29`) · ✅ 페이지 간격 주석 192건 중 21건이 스케일 밖(`GAP-30`)

낮음 4건 — 간격 배수 열 구멍 · 목차 오타 다수 · 히스토리 파일 위치 미기록 · 색 축약 표기
...
**계열은 토큰으로 굳히지 않았습니다.** 원본이 여러 단계에 같은 쓰임새를 적어 두어, 그 중 어느 단계를 고를지에 대한 규칙이 원본에 없기 때문입니다. `$notes.typeSemantic.families` 로 남깁니다.
...
**문서화 규칙도 원본에 있습니다** — 현재 사용 중인 요소는 **포함**, 사용되지 않는 요소는 **제외(히스토리 파일)**, 변경은 `As is / To be` 주석. 레거시를 판단 근거에서 빼는 것이 이 저장소만의 방침이 아니라 **원본 규칙**이었습니다.
...
## 🚧 페이지를 직접 읽었더니 (2026-08-06)

**`🚧` 는 «미완»이지 «빈 페이지»가 아닙니다.** 등록된 스타일·에셋 목록만 보고 «🚧 이니 내용이 없다»고 세어 온 것이 잘못이었습니다(`GAP-32`). 페이지를 한 장씩 열어 `get_metadata` XML 을 저장하고 `tools/parse-figma-page.js` 로 구조화합니다. 읽은 원문은 `data/figma-pages/*.json`, 페이지별 판단은 `data/<이름>-page.json` 으로 따로 냅니다.

**🚧 19쪽 중 4쪽 확인 — 4쪽 모두 내용이 있었습니다.**

| 페이지 | 이전에 적어 둔 것 | 실제 |
|---|---|---|
| `Icon system` 🚧 | 이름 규칙 없음 · 크기 스케일 없음 · 목록 없음 | 셋 다 있음 — `Icon/system/<이름>/<line\|fill>/<크기>` · XL 96/L 56/M 40 · 아이콘 184종 |
| `Elevation system` 🚧 | 정의 없음, EFFECT 스타일 6종이 전부 | Level 0~5 · dp(0/1/4/6/8/12) · X/Y/Blur/Spread/30% 표 · 적용 범위 · 인용 출처 |
| `Radius system` 🚧 | 값 7개 | 쓰임새 라벨 + 원본의 «10 → 8» 미결 메모 5건 + KRDS 형태 가이드 인용 프레임 |
| `Text field` 🚧 | 원본 대기 | 정의 · 유형 4가지 · 유형별 구조와 스페이싱 · 변형 42종 · 페이지 결함 5건 |

### 읽으면서 지킨 것

- **채우지 않은 템플릿 칸을 «내용»으로 세지 않습니다.** `설명글` · `레벨 값` · `Reference` 는 복사만 하고 안 채운 자리입니다. `templateStubs` 로 따로 셉니다.
- **페이지 값으로 토큰을 덮어쓰지 않습니다.** Elevation 페이지 표는 1겹, 라이브러리 `Elevation_2` 는 2겹입니다. 어느 쪽이 맞는지 원본이 말해 주지 않으므로 **양쪽을 나란히 싣고 어긋난다고 적습니다**(`GAP-33`).
- **원본이 붙여 둔 남의 것은 인용으로 표시하고 편입하지 않습니다.** Radius 페이지의 KRDS 형태 가이드, Text field 페이지 Style 절에 남아 있는 Buttons 문장이 그렇습니다.
- **오타를 고치지 않고 셉니다.** `Leverl`(→Level) 20곳 · `Spreas`(→Spread) · `License`/`license`.

---

## 레퍼런스로 메우기 전에 — 두 번 틀린 기록 (2026-08-06)

**아이콘 그리드와 크기 용도를 «원본에 없다»고 보고 레퍼런스(M3·KRDS)로 메웠습니다. 전부 원본에 있었습니다.**

| | 무엇을 틀렸나 | 왜 |
|---|---|---|
| 1차 | `🚧` 이니 내용이 없다고 보고 «이름 규칙 없음 · 목록 없음 · 크기 스케일 없음» | 페이지를 **안 열었습니다** |
| 2차 | 페이지를 «직접 읽었다»고 하고도 «그리드 없음 · 크기 용도 없음» | 열긴 했는데 노드 **이름만** 훑고 본문을 안 읽었습니다 |

2차의 원인은 파서였습니다. `tools/parse-icon-page.js` 가 `Icon/system/*` 노드 이름만 모으고 문서 프레임의 텍스트를 버렸습니다. 강민관이 「그리드 그려져 있고 End cap은 둥글게가 맞아용」이라고 확인해 준 뒤 `tools/parse-figma-page.js` 로 페이지를 통째로 다시 읽었습니다 — 본문 **1,356줄**이 `data/figma-pages/icon-system-full.json` 에 남습니다.

### 원본에 있던 것 — 제작 가이드라인 `00_Size` ~ `08_Color`

| | |
|---|---|
| 프레임 | `24×24` · Live area `20×20` · 세이프존 여백 **최소 2** |
| 키라인 | Square `18×18` · Circle `20×20` · Rectangular `16×20` / `20×16` |
| 스트로크 | **`1.2px`** (허용 오차 `±0`) — 「1.5px 이상은 곡선이 많은 형태(자동차 등)에서 뭉쳐 보이는 착시가 발생해 지양」 |
| 크기별 굵기 | `40:2` · `32:1.6` · `24:1.2` · `20:1` · `16:1` |
| 코너 | `2px` — 「스트로크 두께 대비 약 1.67배로, round cap 기본값과 톤을 맞추는 최소 곡률」 |
| End cap | 기본 **Round** · 교차 지점 예외 **Butt + 1px offset** |
| Off slash | 좌상→우하 · 여백 `0.8` · Stack 겹침 여백 `1.6` |
| 이름 규칙 | `Icon/{카테고리}/{이름}/{스타일}/{크기}` — 원본이 스스로 선언 |

### 그래서 레퍼런스는 «메우는 것»에서 «대조하는 것»으로 강등했습니다

`data/icon-reference.json` 의 `status` 가 `proposal` → **`comparison`** 입니다. 이제 하는 일은 하나 — **원본 문장이 어디서 왔는지 기계로 가르는 것**입니다.

- **KRDS 원문 그대로 2절** — `06_Off slash` · `07_Stack` (겹침 100%)
- **부분 일치 3건** — `00_Size` 2건 · `01_Layout`. `00_Size` 가 부분에 그친 이유는 **원본 오타**(`20ox`→`20px`, `넒은`→`넓은`)가 문장을 끊어 놓아서입니다. 도구가 그 사실도 같이 표시합니다.
- **G car 고유 판단 4가지** — 스트로크 `1.2px`(KRDS 는 `1.6`) · 크기별 굵기 5단계(두 참고에 없음) · End cap 교차 예외 · 세이프존 「최소」 표현

판정은 손으로 「비슷하다」고 적지 않습니다. 문체(`합니다`/`한다`)와 구두점을 지운 뒤 **최장 공통 부분문자열 비율**로 재고, 0.8 이상이면 `verbatim` 입니다.

### 원본에 없는 것 — 채우지 않았습니다

터치 표적 크기 · 20px 미만 라벨 하한 · 색 대비율 수치. 두 참고에는 있으나 원본에 없는 축이라 **그대로 «없음»으로 둡니다.** 원본에 없는 축을 참고로 채우면 그게 오염입니다.

### 검증 — 참고가 원본을 덮어쓰지 못하게

`npm run check` 에서 실제로 걸리는 것들입니다.

- 절 본문이 **원본 텍스트와 글자 그대로** 같은지 (`j.guide[3].lead` 를 바꾸면 → `절 본문이 원본 텍스트와 글자 그대로 일치` 실패)
- 스트로크가 **`1.2px` 인지** — `1.6px`(KRDS)나 `2px`(M3)로 바뀌면 실패
- 원본 안 모순(`GAP-36` 32px 굵기 1.6 vs 1.5)이 **`decided: false` 로 남아 있는지** — `true` 로 바꾸면 실패
- `--gds-icon-*` 중 **px 값 토큰이 0개**인지 (색 시맨틱 2종은 원본 근거라 예외)

### `00_Size` 밖 크기는 유지

`36`(5건) · `50`(1건) · `56`(1건) — 원본 주요 크기 다섯(`16·20·24·32·40`) 밖입니다. **강민관 2026-08-06 「일단 36 50 56은 냅 두시고」.** 정리하지 않고 건수와 함께 기록만 합니다.
...
- **`#00C88C` 는 GDS 원본 `System/Success` 와 HEX 가 같습니다.**
- 본문 서체는 **Pretendard**, 라틴은 **Outfit** 입니다. GDS GDS 서체(`TQ-6` Noto Sans KR)와 다릅니다 — 다른 표면이므로 충돌은 아닙니다. **문서 사이트는 GDS 서체 Noto Sans KR 을 그대로 씁니다** (`TQ-8` 확정 2026-08-06). 견본이 원본과 다른 글꼴로 렌더되면 문서가 스스로를 반증하기 때문입니다.

### 그래서 고친 것

| 무엇이 문제였나 | 어떻게 고쳤나 |
|---|---|
| **히어로가 전면 빨강 그라디언트**, 활성 표시가 빨강 알약. 면적이 과했습니다 | 히어로는 종이 배경 + 원본 색 띠 한 줄로. **크롬 accent 는 빨강 유지** — `Primary/Red 040`(메인 색) · `010`, 다크는 `030` · `080`. G car 의 디자인 시스템이므로 G car 메인 색을 씁니다 (강민관 확정 2026-08-06). 레퍼런스 다섯 곳은 크롬에 브랜드색을 안 쓰지만, 그것과 다른 선택을 한 것입니다 |
...
그래서 «우리가 안 만든 것»이 아니라 «원본에 없는 것»임을 조사로 확인했습니다 — `tools/spacing-census.js`.
...
**값은 나오지만 이름이 하나도 없습니다.** 담고 있는 프레임 이름도 대부분 `Group 2147227…` 같은 자동 이름입니다. 없는 이름을 지어내면 그 순간 이 문서는 원본이 아니라 추측이 됩니다.
...
원본 EFFECT 스타일은 `Elevation_1~6` 만이 아닙니다. **14종** 중 8종이 체계 밖에 있습니다(`GAP-15`).
...
| 축 | 수 | 뜻 |
|---|---|---|
| `elevation` | 6 | 표면의 높이. `--gds-elevation-*` |
| `component` | 7 | 컴포넌트 전용 효과. **높이 스케일이 아니라서 Elevation 에 넣으면 안 됩니다** |
| `material` | 1 | `Frosted Glass` — 그림자가 아니라 **블러**입니다. 다른 축 |
| `deprecated` | 2 | 원본이 `(X)` 로 표시한 것. 토큰에서 제외 |

판정 규칙은 `Elevation_N` / `as-is_`·`(X)` / `BACKGROUND_BLUR` / 나머지 — 손으로 분류하지 않습니다.

**내보낸 것 3종** — `--gds-effect-bottom-sheet-shadow` · `-bottom-floating-button` · `-frosted-glass`.
`level/gold/*` 3종은 값이 아직 측정되지 않아 «미측정»으로 남깁니다.

### 내보내지 않은 것 — `GAP-31`

`Bottom Sheet`(3겹)와 `bottom sheet`(1겹)는 **대소문자만 다른 이름인데 값이 서로 다릅니다.**

| | |
|---|---|
| `Bottom Sheet` | `0/4/8/3` + `0/1/3/0` + `0/0/6/0` (3겹) |
| `bottom sheet` | `0/1/2/0` (1겹) |

값이 다르므로 «오타 중복»으로 합칠 수 없습니다. 토큰 키가 `--gds-effect-bottom-sheet` 하나로 겹치므로 **둘 다 내보내지 않습니다** — 잘못된 값이 나가는 것보다 안 나가는 편이 낫습니다. `check.js` 가 «키가 겹치는 효과를 내보내지 않았는지»를 봅니다.
...
| **Primary 메인** | **`Primary/Red 040` (`#F14950`)** — 원본 설명문대로. 브랜드 레드 `#ED1C24` 와는 다른 값 | 2026-08-04 강민관 |
| **메인 색 접근성** | **별도 처리 없음** — 흰 배경 3.62:1 로 본문 AA 미달이나 그대로 사용 | 2026-08-04 강민관 |
| **원본 기준** | **«GDS (그린카 디자인 시스템)» 라이브러리 + ✅ 페이지** — 레거시 라이브러리는 판단 근거에서 배제 | 2026-08-05 강민관 |
| **GDS 폰트** | **Noto Sans KR 단일** | 2026-08-04 강민관 |
...
사이트도 GDS 폰트로 렌더됩니다 — `build/font.js` 가 빌드 시 모든 페이지의 본문 폰트 선언을 GDS 폰트로 교체하고, `assets/fonts/` 의 서브셋 woff2(weight 당 약 40 KB)를 base64 로 임베드합니다. 외부 폰트 요청은 없습니다. 서브셋 재생성은 `tools/font/README.md` 참고 — `npm run check` 의 `서브셋 글리프 누락 없음` 이 실패하면 필요합니다.
...
```bash
npm run build    # 원본 사이트 + /gds/decisions
npm run check    # 원본·결정·산출물 기계 대조 (167개 항목)
```

| 산출물 | 내용 |
|---|---|
| `/gds/decisions` | 결정 기록 — 원본 기준 · 확정 결정 · 모자란 곳 12건 |
| `data/gds-library.json` | 원본 인벤토리 — GDS 라이브러리 60종 + 효과·텍스트·반경 |
...
| 행간 | 고정 px 를 쓰던 `Text styles/*` 는 **레거시 라이브러리 소속**이라 무시합니다. 원본 `noto_sans/*` 는 `lineHeight 100` — `TQ-7` **확정: `Auto` 유지** |
