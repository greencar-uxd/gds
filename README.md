...
| **CQ-5** 이름 충돌 | 남은 4종 확정 — `Default Btn` 두 값 모두 `Navy/Navy 060` · `Second Navy/navy090` 은 값별로 `Navy 100`/`Navy 080` 분리 · `First Green/g600_G press` 두 값 모두 `Primary/Red 060` · `Map Marker/active` 이름 폐기 | 2026-08-05 |
| **CQ-6** 정본에 없는 색 | 묶음 8개 중 **5개 확정** — OC-2 전량 폐기 · OC-3 GDS 밖으로 분리 · OC-5·OC-6 일괄 치환. 처분 **49종 / 133개 스타일** | 2026-08-05 |
| **CQ-9** 알파 토큰화 | **A 채택** — `System/Dim Layer 060`(`#00000099`) · `080`(`#000000CC`) 신설, `Info Box BG` 는 060 으로 흡수 | 2026-08-05 |
| **CQ-10** 역할 별칭 | **C 채택** — `color/text/bolder` 폐기(레거시 사용 0건). 시맨틱 계층 자체는 원본 Buttons 에서 이미 쓰고 있음 | 2026-08-05 |
...
대조 중 발견한 `TQ-6`(Time picker 서체)은 **확정됐습니다** — 서체는 **Noto Sans KR 단일**, 크기는 정본 `Title 1`(24px). 파일 안 다른 페이지와 레거시 스타일에 실재하는 **Rubik 50px** 3단계는 정본에 편입하지 않습니다. (강민관 확정 2026-08-05)
...
원본 `.fig` 자체의 기록 17건(스와치 캡션 HEX 오류 2건, `Success` 이름 중복, `#00000` 5자리 오타 3곳, 복붙 잔재 레이어명, `Info_Box` 스와치 값 미적용, 이름 속 제어문자 `U+0008`·`U+2028`)은 `data/color-decisions.json` 의 `sourceDefects` 에 노드 ID 와 함께 기록만 해 뒀습니다. 토큰 값에는 영향이 없습니다.

### 색 결정 진행 (2026-08-05)

| | 이전 | 지금 |
|---|---|---|
| 정리된 레거시 스타일 | 478개 (73.7%) | **587개 (90.4%)** |
| 개별 판단 필요 | 171개 | **62개** |
| 정본에 없는 색 중 개별 판단 | 74종 | **26종** |
| 열린 안건 | 6건 | **1건** (`CQ-11`) |

처분 내역은 `data/color-decisions.json` 의 `orphanDispositions` **49종**에 색·치환 대상·해당 스타일 이름·사유가 모두 들어 있습니다. `action: "external"` 13종은 iOS UIKit·Material·제휴사가 소유한 색이라 치환하지 않고 시스템 밖으로 분리한 것입니다.

남은 것은 **묶음 3개**(`OC-4` 네이비 8종 · `OC-7` 뱃지·마커 7종 · `OC-8` 그 외 8종)와 **색이 곧 정보라 남긴 3종**(`#00AD73` 지도 활성 마커 · `#00AD79` 로고 뱃지 · `#FCF8E8` 등급 표시)입니다. 전부 실제 화면을 봐야 정할 수 있는 것들입니다.
...
| 행간 세 방식 | 원본 타이포 변수에 `100` · 고정 px · 배수 `1.5` 가 공존합니다. `TQ-7` **확정 — `Auto` 유지**, 저장소는 세 방식을 `Auto` 로 수렴시킵니다 (강민관 2026-08-05) |
