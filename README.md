...
- 본문 서체는 **Pretendard**, 라틴은 **Outfit** 입니다. GDS 정본 서체(`TQ-6` Noto Sans KR)와 다릅니다 — 다른 표면이므로 충돌은 아니지만 «문서 사이트는 어느 쪽 옷을 입는가»는 아직 정하지 않았습니다.

### 그래서 고친 것

| 무엇이 문제였나 | 어떻게 고쳤나 |
|---|---|
| **크롬이 브랜드 빨강**이었습니다. 사이드바 활성 표시가 빨강 그라디언트 알약, 히어로가 전면 빨강. 색 문서가 자기 색에 물들면 스와치가 UI 의 일부처럼 보입니다 | 크롬 accent 를 **정본 `Navy/Navy 060` · `010`**(다크는 `030` · `080`)으로. 값은 우리 토큰 그대로이고 `check.js` 가 정본과 대조합니다. 히어로는 종이 배경 + 정본 색 띠 한 줄 |
