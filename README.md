...
- **등록된 에셋·스타일 목록만 보지 않습니다.** 페이지를 직접 엽니다.
  `🚧` 는 «내용이 없다»는 뜻이 아닙니다 — Icon system 페이지가 🚧 인데도 이름 규칙·아이콘 184종·크기 단계를 전부 갖고 있었고,
  목록만 보고 «없다»고 적은 3건을 철회했습니다(GAP-22 정정).
...
중간 3건 — ✅ 인데 미해결 메모가 남은 페이지(`GAP-25`) · Title 8단계 중 2개가 라이브러리에 없음(`GAP-29`) · ✅ 페이지 간격 주석 192건 중 21건이 스케일 밖(`GAP-30`)
...
## 원본에 없는 것을 레퍼런스로 메울 때 (2026-08-06)

페이지를 다 읽고도 **원본 어디에도 없는** 자리가 남습니다. 그때만 레퍼런스를 봅니다 — `data/icon-reference.json` 이 첫 사례입니다(`GAP-22` 잔여 2건).

**메웠다고 «있는 것»이 되지는 않습니다.** «원본에 없음»은 그대로 두고 참고 후보를 옆에 답니다. 사이트에서도 절을 갈라 놓았습니다.

| | 채운 것 | 출처 |
|---|---|---|
| **아이콘 그리드** | 24px 캔버스 · Live area `20×20` · 사방 여백 `2px` · 키라인(정사각 18 · 원 Ø20 · 세로 20×16 · 가로 16×20) · 모서리 2px · 선 굵기 1.5~2 | [M3 Designing icons](https://m3.material.io/styles/icons/designing-icons) · [KRDS 아이콘](https://www.krds.go.kr/html/site/style/style_06.html) |
| **크기 단계 용도** | `16` 조밀 · `20` 조밀 레이아웃 · `24` **기본** · `32` 강조(Bottom navigation) · `40` 주 동작 강조 | [KRDS 아이콘](https://www.krds.go.kr/html/site/style/style_06.html) · [M3 Applying icons](https://m3.material.io/styles/icons/applying-icons) |

**지어낸 배정이 아닌 이유.** 원본이 선언한 크기 중 `16·20·24·32·40` 이 KRDS 의 주요 크기 다섯과 **정확히 같습니다.** 24 가 기본이라는 판정도 참고가 아니라 원본 분포에서 나옵니다 — 선언 182건이 24px, 다음이 20px 7건. 32px 자리는 아예 원본 실측입니다(Bottom navigation ✅ 아이콘 32×32 · 굵기 1.5).

### 지키는 규칙 — 검증 12개가 강제합니다

- 항목마다 **출처 URL + 원문 인용** 없으면 빌드가 섭니다.
- 참고가 적은 «원본 건수»를 페이지 읽기 결과와 **대조**합니다. 999 로 바꿔 보면 `크기 24 의 원본 건수가 페이지 읽기와 다릅니다 — 제안 999 / 페이지 182` 로 섭니다.
- 제안에 안 담긴 크기(`36`·`50`·`56`)를 **숨길 수 없습니다.** 하나라도 빠지면 `제안이 원본 선언 크기를 전부 다루지 않습니다` 로 섭니다.
- **치수 토큰으로 나가지 못합니다.** `--gds-icon-*` 중 px 값을 가진 토큰이 하나라도 생기면 실패합니다. (색 시맨틱 2종은 원본 근거라 예외)
- **출처끼리 갈리면 갈린 채로 둡니다** — end cap 을 KRDS 는 둥글게, M3 는 각지게 권합니다. 하나로 정하지 않고 미결로 남겼습니다.

**참고로도 안 메워진 것 3건** — 그리드가 원본 파일에 이미 그려져 있는지(metadata 로는 안 보임) · end cap 둥글게 vs 각지게 · `36`·`50`·`56` 을 살릴지.
